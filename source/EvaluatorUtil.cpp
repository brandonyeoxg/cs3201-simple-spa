...
std::vector<std::string> EvaluatorUtil::getCommonProgLineAndConstant(std::vector<std::string> t_allConstants, int t_maxProgLine) {
  std::vector<std::string> commonProgLineAndConstant;

  for (auto& constant : t_allConstants) {
    if (std::stoi(constant) > t_maxProgLine) {
      commonProgLineAndConstant.push_back(constant);
    }
  }
