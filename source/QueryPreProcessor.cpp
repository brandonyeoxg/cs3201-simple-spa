...
bool QueryPreProcessor::tokenizeQuery(std::string t_queryInput) {
    ...
    if ((sTInt1 == sTInt2 && sTInt1 != 0) || (sTName1 == sTName2 && sTName1 != "_")) {
      ...
      for (auto k = m_grammarVector.begin(); k != m_grammarVector.end(); k++, counterK++) {
        ...
        if (sTName1 == grammarName) {
          ...
          if (designAbstractionEntity == "Parent" && g1.getType() == queryType::GType::VAR
            || designAbstractionEntity == "Parent*" && g1.getType() == queryType::GType::VAR) {
            return false;
          }
  ...
  if (patternStatement == "") {
    ...
    if (grammarPatternLeft.getType() == queryType::GType::EXPR) {
